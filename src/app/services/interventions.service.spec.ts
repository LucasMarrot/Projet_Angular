...
describe('InterventionsService', () => {
  ...
  it('should fetch all interventions', () => {
    const mockInterventions: Intervention[] = [
      { id: 1, date: new Date(), description: 'Test intervention', duree: 2, idChaudiere: 1 },
      { id: 2, date: new Date(), description: 'Another intervention', duree: 1, idChaudiere: 2 },
    ];
  ...
  it('should add a new intervention and return it', () => {
    ...
    spyOn(service, 'getInterventions').and.returnValue(of(mockInterventions));

    service.addIntervention(newIntervention).subscribe((intervention) => {
      expect(intervention).toEqual(addedIntervention);
    });

    const postReq = httpMock.expectOne('http://localhost:3000/Intervention');
